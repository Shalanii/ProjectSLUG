...
<div class="container">
	<div class="jumbotron" style="background: linear-gradient(to bottom left, #00ccff 23%, #cc33ff 100%);">
        <div class="heading-section ftco-animate">
	        <h2 class="mb-4 text-center">Elle Game Reports</h2>
	    </div>
        <div class="d-flex justify-content-center">
        <form class="form-inline" action="/search5" method="post">
        {{csrf_field()}}
...
<div class="container" align="center">
    ...
    @foreach($r as $item)
    <div class="col-md-6">
	    <div class="scoreboard scoreboard-2 slash" style="background:url(/img/image/ell.jpg);background-size:cover;min-height:auto;" data-stellar-background-ratio="0.5">
			<div class="divider ftco-animate text-center"><span style="color:#dae44d;font-size:16px;">{{$item->Date}} | {{$item->Round}}</span></div>
				<div class="divider ftco-animate text-center mb-4"><span class="x">Gender - {{$item->Gender}}</span></div>
			        ...
			        <div class="text-center ftco-animate">
                        ...
                        @else
                            <h6 style="color:#cc9b17;font-size:17px;">
						
						@if($item->toss==$item->Uni1)
							<p>Toss Won : {{$item->Uni1}}</p>
						{{$item->UniName}} Won the Match
						@elseif($item->toss==$item->Uni2)
							<p>Toss Won : {{$item->Uni1}}</p>
						{{$item->UniName1}} Won the Match
						@endif
								</h6>
                        @endif
				          					    </div>
