...
@section('content')
<center><h3>Elle Result Add</h3></center>
<div class="container">
  <div class="col-sm-12">
    <div class="card">
      <div class="card-body">
      <?php //form Validation ?>
        <form action="/saveElleResult" method="post">
          {{csrf_field()}}
          <div class="col-md-12">
            @if(isset($error))
	...
	<div class="col-md-3">
              <div class="form-group{{ $errors->has('toss') ? ' has-error' : '' }}">
              <label>Toss</label>
		            <select name="toss" class="form-control" >
                    <option></option>
                  @foreach($uniIDarray as $data)
                    <option value="{{$data->UniID}}">{{$data->UniID}}</option>
                  @endforeach
                ...
                @if ($errors->has('toss'))
                    <span class="help-block">
                        <strong>{{ $errors->first('toss') }}</strong>
                    </span>
                @endif
              </div>
            </div>
    ...
    <div class="card">
      ...
      <table class="table table-hover table-responsive">
          ...
          <tr>
              ...
              <td>{{$task->Gender}}</td>
              <td>
                  <a href="/deleteElleResult/{{$task->MatchNo}}/{{$task->Gender}}" class="btn btn-sm btn-danger">Delete</a></td>   
          </tr>
          @endforeach
          @endif
      </table>
